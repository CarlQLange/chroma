...
// Colour represents an RGB colour or a terminal palette colour index.
//
// Encoding:
//   - 0 = unset
//   - > 0 = RGB colour (value is RGB + 1)
//   - < 0 = terminal palette index (value is -(index + 1))
type Colour int32
...
// NewTermColour creates a Colour that references a terminal palette index (0-255).
// In theme syntax this is written as "term-N". When rendered to a terminal,
// these emit ANSI index escape codes so the output adapts to the user's palette.
func NewTermColour(index uint8) Colour {
	return Colour(-(int32(index) + 1))
}

// IsTermColour returns true if this colour represents a terminal palette colour.
func (c Colour) IsTermColour() bool { return c < 0 }

// TermIndex returns the terminal palette index (0-255). Only valid if IsTermColour() is true.
func (c Colour) TermIndex() uint8 { return uint8(-(int32(c) + 1)) } //nolint:gosec
...
// ParseColour in the forms #rgb, #rrggbb, #ansi<colour>, #<colour>, or term-N.
// Will return an "unset" colour if invalid.
func ParseColour(colour string) Colour {
	if strings.HasPrefix(colour, "term-") {
		n, err := strconv.ParseUint(colour[5:], 10, 8)
		if err != nil || n > 255 {
			return 0
		}
		return NewTermColour(uint8(n)) //nolint:gosec
	}
...
func (c Colour) String() string {
	if c.IsTermColour() {
		return fmt.Sprintf("term-%d", c.TermIndex())
	}
	return fmt.Sprintf("#%06x", int(c-1))
}

func (c Colour) GoString() string {
	if c.IsTermColour() {
		return fmt.Sprintf("TermColour(%d)", c.TermIndex())
	}
	return fmt.Sprintf("Colour(0x%06x)", int(c-1))
}

// Red component of colour. For terminal palette colours, returns the default palette value.
func (c Colour) Red() uint8 {
	if c.IsTermColour() {
		return termPalette[c.TermIndex()][0]
	}
	return uint8(((c - 1) >> 16) & 0xff) //nolint:gosec
}

// Green component of colour. For terminal palette colours, returns the default palette value.
func (c Colour) Green() uint8 {
	if c.IsTermColour() {
		return termPalette[c.TermIndex()][1]
	}
	return uint8(((c - 1) >> 8) & 0xff) //nolint:gosec
}

// Blue component of colour. For terminal palette colours, returns the default palette value.
func (c Colour) Blue() uint8 {
	if c.IsTermColour() {
		return termPalette[c.TermIndex()][2]
	}
	return uint8((c - 1) & 0xff) //nolint:gosec
}
...
// termPalette maps terminal palette indices 0-255 to default RGB values.
// These match the standard xterm-256color palette and are used as fallback
// values when rendering terminal palette colours to non-terminal outputs (HTML, SVG).
var termPalette = [256][3]uint8{
